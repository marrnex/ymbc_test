   ...
   //     fprintf(fp, "%f %f\n", i*0.1, (i*0.1)*perp(yob[xmin],xob[xmin],yob[xmin2],xob[xmin2],(yob[xmin]+yob[xmin2])/2,(xob[xmin]+xob[xmin2])/2)+perp2(yob[xmin],xob[xmin],yob[xmin2],xob[xmin2],(yob[xmin]+yob[xmin2])/2,(xob[xmin]+xob[xmin2])/2));
      ...
      switch(step){
        case 0://最初の直進
        		...
        		while(1){
        			...
        			for(i = 0; i < urg_data->size; ++i) {
	        			if((i>360)&&(i<408)&&(urg_data->data[i]<2000)&&(urg_data->data[i]>1700)){//前方障害物
	              			if(maxobs<=urg_data->data[i]){//前方の中で最も距離のある物体を導出
	         				obslf=1;                
        ...
        case 7://ちょっと前進(ポールを認識しないため)
                ...
                usleep( 1000000 );
                  Spur_stop_line_FS( 0.8,-(pole-1.6) , 0 );
                usleep( 3000000 );
...
*/
      ...
      break;
        ...
        default:Spur_stop();usleep(100000); break;
          ...
          fprintf(fp, "%f %f\n", yob[test1], xob[test1]);
          fprintf(fp, "%f %f\n", yob[test2], xob[test2]);
          fprintf(fp, "%f %f\n", ( (yob[test1]+yob[test2])/2),((xob[test1]+xob[test2])/2));
			fputs("e\n", fp);          
          puts("");
          printf("test1: x:%f y:%f\n",xob[test1],yob[test1]);
