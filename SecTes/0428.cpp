...
int main(int argc, char **argv) {
  ...
  int obs=0,obsr=0,obsl=0,obslf=0,obslb=0,obsrf=0,obsrb=0;
  ...
  while(!gIsShuttingDown) {//多分ここで描画とかが起きてる
    ...
    //  fputs("plot '-'\n", fp);
      for(i = 0; i < urg_data->size; ++i) {//取得できたデータの個数だけループ
        ...
        if((i>341)&&(i<427)&&(urg_data->data[i]<600))//前方障害物
          obs=1;
        if((i>631)&&(i<649)&&(urg_data->data[i]<400))//左障害物
          obsl=1;
        if((i>119)&&(i<137)&&(urg_data->data[i]<400))//右障害物
          obsr=1;
        if((i>205)&&(i<223)&&(urg_data->data[i]<400))//右前障害物
          obsrf=1;
        if((i>44)&&(i<53)&&(urg_data->data[i]<400))//右後ろ障害物
          obsrb=1;
        if((i>545)&&(i<563)&&(urg_data->data[i]<400))//左前障害物
          obslf=1;
        if((i>710)&&(i<725)&&(urg_data->data[i]<400))//左後ろ障害物
          obslb=1;
      ...
      switch(step){
        case 0: Spur_stop_line_LC( x, y, 0 ); 
                ...
                break;//直進
        case 1: Spur_spin_GL(-M_PI/2);//右90度回転
                if(obsl==0){
                  usleep( 100000 );//待機
                }else
                step++;
                break;//方向転換
        case 2:Spur_stop_line_LC( x, y, 0 );
               if(obsr)
         break;//
        case 3: Spur_stop();break;
      }
//      fputs("e\n", fp);
